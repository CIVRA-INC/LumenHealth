...
export function sendInvitation(
  ...
  // Block duplicate pending invitations to the same email in the same clinic
  const existing = invitationStore.findByEmail(clinicId, body.email);
  if (existing?.status === "pending") {
    return { error: "INVITATION_ALREADY_PENDING", message: "a pending invitation already exists for this email" };
  }

  const existingIdentity = identityStore.findByEmail(body.email);
  if (existingIdentity) {
    return { error: "STAFF_ALREADY_EXISTS", message: "this email is already registered as staff" };
  }
...
export async function acceptInvitation(
  ...
  if (invitation.status === "accepted") {
    return { error: "INVITATION_ALREADY_ACCEPTED", message: "invitation has already been accepted" };
  }
  if (invitation.status === "revoked") {
    return { error: "INVITATION_REVOKED", message: "invitation has been revoked" };
  }

  // ← GAP: expiry check missing here
...
export function revokeInvitation(
  ...
  if (invitation.status === "accepted") {
    return { error: "INVITATION_ALREADY_ACCEPTED", message: "cannot revoke an already accepted invitation" };
  }
